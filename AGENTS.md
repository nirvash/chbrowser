...
5. **PR 作成** — 動作確認が取れたらブランチを push して
   `gh pr create` で PR を出す (base = 自フォーク `nirvash/chbrowser` の main)
6. **レビュー対応後マージ** — レビュー指摘に対応し、指摘が解消されてからマージする。
   GitHub 上では Squash and merge を使う。ローカルで main に取り込む場合も
   `git merge --squash <branch>` + 単一コミットとし、マージコミットを履歴に残さない
