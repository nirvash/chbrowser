...
このリポジトリは、[pawapawa041950/chbrowser](https://github.com/pawapawa041950/chbrowser) をフォークして大幅に拡張した版です。

- ふたばちゃんねるの板・カタログ・スレ表示と引用ツリー
- 前後スレナビゲーション、未読・新着表示、自動更新
- メディアの先読み・保存・到達可能フィルタ・動画再生機能
- MCP／AIエージェントによるスレッド操作とお気に入り管理
- ペイン操作、ページズーム、スクロール復元などの表示・操作性改善
...
- **(実験的)MCPサーバ** 5chの板一覧・スレッド一覧・スレッド内容の取得に加え、お気に入りのフォルダ作成・追加・削除・移動・リネームをMCPで操作可能
...
| ファイル名 | 用途 | 未配置時の動作 |
|---|---|---|
| `image-404.png` | 削除済み画像 / 取得失敗動画スロットの代替画像。正方形・透過 PNG 推奨 (512px 程度)。スロットクリックで再試行 | 「読み込み失敗 — クリックで再試行」テキスト表示 |
